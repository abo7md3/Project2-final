...
<body onload="pageloaded()">
  ...
  City:
  <select>
    <option disabled selected>-- Select So --</option>
    <?php
        include "db.php";  // Using database connection file here
        $records = mysqli_query($db, "SELECT * From So");  // Use select query here 

        while($data = mysqli_fetch_array($records))
        {
            echo "<option value='". $data['student_outcome'] ."'>" .$data['student_outcome'] ."</option>";  // displaying data in option menu
        }	
    ?>  
  </select>
</form>
...
<?php mysqli_close($db);  // close connection ?>
    ...
    <script>
        function getInputValue(){
            ...
            var inputVal = document.getElementById("section_input").value;
